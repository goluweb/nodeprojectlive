...
app.get('/show_teacher',rolePermission('viewTeacher'), async(req,res)=>{
    ...
    if(req.session.user.roles_id.roles === 'admin'){
     teacher = await teacterModel.find().sort('-_id').populate('assign_course','_id playlist').populate('user_id','name')
     playlist = await playListModel.find().sort('-_id').select('_id playlist')
    }else{
         teacher = await teacterModel.find({user_id:req.session.user._id}).sort('-_id').populate('assign_course','_id playlist').populate('user_id','name')
...
app.post('/add_teacher',rolePermission('addTeacher'),imageUpload.single('image'),async(req,res)=>{
   const {name,mobile,email,password,assign_course,exprience,address} = req.body;
console.log(req)
teacterModel.create({
...
app.post('/edit_teacher/:id',rolePermission('editTeacher'),imageUpload.single('image'),async (req,res)=>{
    const {id}=req.params;
    const {name,mobile,email,password,assign_course,exprience,address,imageUnlink} = req.body;
    console.log(req)
    let imageUpload='';
